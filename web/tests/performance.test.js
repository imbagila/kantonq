...
async function runLighthouse(browser, url, pageName = "index") {
  ...
  try {
    ...
    Object.values(categories).forEach((cat) => {
      ...
      console.log(
        `│ ${emoji} ${cat.title.padEnd(20)} ${bar} ${String(score).padStart(3)}% │`,
      );
    });
    ...
    Object.entries(vitals).forEach(([key, { name }]) => {
      ...
      if (audit) {
        ...
        console.log(
          `│ ${scoreEmoji} ${name.padEnd(35)} ${String(value).padStart(10)} │`,
        );
      }
    });
...
async function runDevToolsPerformance(browser, url, pageName = "index") {
  ...
  console.log(
    `│ DNS Lookup                       ${formatTime(performanceTimings.dnsLookup).padStart(20)} │`,
  );
  console.log(
    `│ TCP Connection                   ${formatTime(performanceTimings.tcpConnection).padStart(20)} │`,
  );
  console.log(
    `│ Server Response (TTFB)           ${formatTime(performanceTimings.serverResponse).padStart(20)} │`,
  );
  console.log(
    `│ First Paint                      ${formatTime(performanceTimings.firstPaint).padStart(20)} │`,
  );
  console.log(
    `│ First Contentful Paint           ${formatTime(performanceTimings.firstContentfulPaint).padStart(20)} │`,
  );
  console.log(
    `│ DOM Content Loaded               ${formatTime(performanceTimings.domContentLoaded).padStart(20)} │`,
  );
  console.log(
    `│ DOM Complete                     ${formatTime(performanceTimings.domComplete).padStart(20)} │`,
  );
  console.log(
    `│ Page Load                        ${formatTime(performanceTimings.pageLoad).padStart(20)} │`,
  );
  console.log(
    "├─────────────────────────────────────────────────────────────┤",
  );
  ...
  console.log(
    `│ JS Heap Used                     ${formatBytes(metricsMap.JSHeapUsedSize || 0).padStart(20)} │`,
  );
  console.log(
    `│ JS Heap Total                    ${formatBytes(metricsMap.JSHeapTotalSize || 0).padStart(20)} │`,
  );
  console.log(
    `│ DOM Nodes                        ${String(Math.round(metricsMap.Nodes || 0)).padStart(20)} │`,
  );
  console.log(
    `│ Layout Count                     ${String(Math.round(metricsMap.LayoutCount || 0)).padStart(20)} │`,
  );
  console.log(
    `│ Style Recalc Count               ${String(Math.round(metricsMap.RecalcStyleCount || 0)).padStart(20)} │`,
  );
  console.log(
    "├─────────────────────────────────────────────────────────────┤",
  );
  ...
  console.log(
    `│ Document Transfer Size           ${formatBytes(performanceTimings.transferSize).padStart(20)} │`,
  );
  console.log(
    `│ Document Encoded Size            ${formatBytes(performanceTimings.encodedBodySize).padStart(20)} │`,
  );
  console.log(
    `│ Document Decoded Size            ${formatBytes(performanceTimings.decodedBodySize).padStart(20)} │`,
  );
  console.log(
    "└─────────────────────────────────────────────────────────────┘",
  );
  ...
  console.log(
    `   - devtools-trace-${pageName}.json (Open in Chrome DevTools > Performance)`,
  );
...
async function runPerformanceTests() {
  ...
  console.log(`\n🎯 Base URL: ${BASE_URL}`);
  console.log(
    `📄 Pages to test: ${PAGES_TO_TEST.map((p) => p.path).join(", ")}`,
  );
  ...
  try {
    ...
    for (const page of PAGES_TO_TEST) {
      ...
      const devtoolsResults = await runDevToolsPerformance(
        browser,
        fullUrl,
        page.name,
      );
    ...
    for (const result of allResults) {
      const perfScore = Math.round(
        result.lighthouse.categories.performance.score * 100,
      );
      const a11yScore = Math.round(
        result.lighthouse.categories.accessibility.score * 100,
      );
      const emoji = perfScore >= 90 ? "🟢" : perfScore >= 50 ? "🟡" : "🔴";
      const pageName = result.path.padEnd(15);
      console.log(
        `│ ${emoji} ${pageName} Perf: ${String(perfScore).padStart(3)}%  A11y: ${String(a11yScore).padStart(3)}%  Load: ${formatTime(result.devtools.loadTime).padStart(8)} │`,
      );
    }
    ...
    if (error.message.includes("net::ERR_CONNECTION_REFUSED")) {
      ...
      console.error(
        "   Or specify a different URL: bun run test:perf --url=https://example.com",
      );
    }
